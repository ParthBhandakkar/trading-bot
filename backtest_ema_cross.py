...
for i in range(backCandles, len(df)-n2):
    ...
    #!!!! parameters
    if ema_5_last > ema_8_last > ema_13_last and ema_5_second_last < ema_8_second_last:  # and df.RSI[row]<30
        signal[i] = 1
    elif ema_5_last < ema_8_last < ema_13_last and ema_5_second_last > ema_8_second_last:  # and df.RSI[row]>70
        signal[i] = -1
    else:
        signal[i] = 0
