...
while True:
    df = _get_historical_data_('ETHUSDT', '1h', '150')
    # df = df.iloc[:-1,:]
    open_ = df['Open']
    high = df['High']
    low = df['Low']
    ...
    if color == 'red':
        ...
        if maxLowPrice > sma_low_last and close_last > high_second_last > ht and open_second_last > ht and close_second_last > ht:
            color = 'green'
        else:
            color = 'red'
    elif color == 'green':
        minHighPrice = min(high[-3], high[-2], high[-1])
        if minHighPrice < sma_high_last and close_last < low_second_last < ht and open_second_last < ht and close_second_last < ht:
            color = 'red'
        else:
            color = 'green'
    # print('maxlowprice = ', maxLowPrice, 'maxhighprice = ', minHighPrice, 'smalow =', sma_low_last, 'smahigh =', sma_high_last )
    if color == 'green' and actions[-1] != 'buy':
        action = 'buy'
        ...
        print("------------------------------------------")
    elif color == 'red' and actions[-1] != 'sell':
        # and last_adx > 24 and last_minus_di > last_plus_di
        action = 'sell'
        actions.append(action)
        print(now, ht, color)
        print(action)
