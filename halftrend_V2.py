...
while True:
    now = datetime.now()
    if (now.minute ==16 or now.minute %15 == 0):
        ...
        if color == 'red':
            ...
            if maxLowPrice > sma_high_last and close_last < low_second_last:
                color = 'green'
            else:
                color = 'red'
        elif color == 'green':
            minHighPrice = min(high[-3], high[-2], high[-1])
            if minHighPrice < sma_low_last and close_last > high_second_last:
                color = 'red'
            else:
                color = 'green'
        ht = up[-1] if trend[-1] == 0 else down[-1]
        print(df.index[-1], ht, color)
        if color == 'green' and last_adx > 25 and last_plus_di > last_minus_di:
            action = 'buy'
            print(action)
            print('plusdi = ', last_plus_di, 'minusdi = ', last_minus_di, 'adx = ', last_adx)
            print("------------------------------------------")
        elif color == 'red' and last_adx > 25 and last_minus_di > last_plus_di :
            action = 'sell'
            print(action)
            print('plusdi = ', last_plus_di, 'minusdi = ', last_minus_di, 'adx = ', last_adx)
            print("------------------------------------------")
