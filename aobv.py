...
def aobv(close, volume, fast=None, slow=None, max_lookback=None, min_lookback=None, mamode=None, offset=None, **kwargs):
    while True:
        ...
        buy_list = obv_long.to_list()
        sell_list = obv_short.to_list()
        ...
        last_value_buy = buy_list[-1]
        last_value_sell = sell_list[-1]

        if last_value_buy == 1 and last_close > second_last_close:
            return'buy'
            break
        elif last_value_sell == 1 and last_close < second_last_close:
            return'sell'
            break
        else :
            return'no'
