...
now = datetime.now()
_get_historical_data_('ETHUSDT', '15m', '1500')
df = df1
...
def aroon(df):
    global df_aroon
    df_aroon = df.copy()
    df_aroon['aroon_up'] = 100 * df1.High.rolling(14 + 1).apply(lambda x: x.argmax()) / 14
    df_aroon['aroon_down'] = 100 * df1.Low.rolling(14 + 1).apply(lambda x: x.argmin()) / 14
...
for i in range(backCandles, len(df)-n2):
    ...
    #!!!! parameters
    if close_last > vwap_last and open_last > vwap_last and open_fifth_last > close_fifth_last and open_fourth_last > close_fourth_last \
            and open_third_last > close_third_last and open_second_last > close_second_last and close_last > open_last:
        signal[i] = 1
    elif close_last < vwap_last < open_last and open_fifth_last < close_fifth_last and open_fourth_last < close_fourth_last \
            and open_third_last < close_third_last and open_second_last < close_second_last and close_last < open_last:
        signal[i] = -1
    else:
        signal[i] = 0
...
class MyCandlesStrat(Strategy):
    ...
    def next(self):
        ...
        if self.signal1==1:
            ...
            tp1 = self.data.Close[-1] + (0.02*self.data.Close[-1])
